...
import { AuditSource, createAuditsSource, createSampleSource } from './data/AuditSource';
    ...
    public updateView(context: ComponentFramework.Context<IInputs>): React.ReactElement {
        ...
        if (!host.readable) {
            ...
            if (sample) {
                ...
                resolve = this.memo(key, () => Promise.resolve(createSampleSource(sample, pageSize)));
            } else {
                mode = 'bad-sample';
                key = mode;
            }
            ...
            const { webAPI, table, recordId, clientUrl } = host;

            resolve = this.memo(key, () => Promise.resolve(createAuditsSource({ webAPI, clientUrl, recordId, table, pageSize })));
        }
